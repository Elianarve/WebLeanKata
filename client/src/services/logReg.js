...
export const registerUser = async (name, email, password) => {
  try {
    const response = await axios.post('http://localhost:5000/auth/register', { // Realiza una solicitud POST al endpoint de registro
      name,
      email,
      password,
    });
...
export const loginUser = async (email, password) => {
  try {
    const response = await axios.post('http://localhost:5000/auth/login', { // Realiza una solicitud POST al endpoint de inicio de sesión
      email,
      password,
    });
